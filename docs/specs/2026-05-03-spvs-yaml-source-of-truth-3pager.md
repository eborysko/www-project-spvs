...
Compound change tags from CONTRIBUTING.md (`[MODIFIED, MOVED FROM x.y.z]`, `[ADDED, SPLIT FROM x.y.z]`) are now structured metadata fields, queryable by tooling and immune to typo-induced parsing errors. Placeholder CSV rows used to reserve deleted/moved id numbers are dropped in MVP — the YAML id sequence is allowed to have gaps. First-class tombstone YAML files (`metadata.status: moved | deleted | deleted_merged_to`) are reserved for a post-MVP supplement that needs explicit deletion semantics; the schema and referential validator already accommodate them.
